# Documentation

This directory contains public documentation for `battery-data-standard`.

## User Documentation

- [Python API reference](api-reference.md)
- [Supported formats](supported-formats.md)
- [Export template](export-template.md)
- [Schema compatibility](schema-compatibility.md)
- [Diagnostics and audit reports](diagnostics.md)
- [Current sign convention](current-sign.md)
- [Step and cycle semantics](step-cycle-semantics.md)
- [BDF compatibility status](bdf-compatibility.md)
- [Warning and issue codes](warning-codes.md)
- [Ecosystem integrations](integrations.md)
